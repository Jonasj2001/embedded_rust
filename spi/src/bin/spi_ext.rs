...
use hal::{pac, prelude::*, spi, gpio};


use cortex_m_rt::entry;

fn read_spi<SPI: spi::Instance, const BIDI: bool, const P: char, const N: u8, MODE>(spi: &mut spi::Spi<SPI, BIDI>, _cs: &mut gpio::Pin<P, N, gpio::Output<MODE>>) -> Option<u8>  {
   while spi.is_busy() {}
   spi.send(0x0).unwrap();
   while spi.is_busy() {}
   match spi.read_nonblocking() {
        Ok(w) => Some(w),
        _ => None,
   }
...
fn main() -> ! {
    ...
    for _ in 0..20 {
        match read_spi(&mut spi1, &mut cs) {
